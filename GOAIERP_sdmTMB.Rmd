...
```{r}
ggplot()+
  geom_sf(data = cv_all_years[cv_all_years$box_id < 92,], aes(fill = cv))+
...
write.csv(x = out, file = paste0("output/biomass/",goaierp_data$name[1],"_",goaierp_data$stage[1],"_GOAIERP.csv"), row.names = FALSE)
```
...
```{r}
write.csv(x = val, file = paste0("output/validation/",goaierp_data$name[1],"_",goaierp_data$stage[1],"_GOAIERP.csv"), row.names = FALSE)
```
